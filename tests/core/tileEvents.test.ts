...
describe('rollTileEvent', () => {
  it('grass / road / lake / rainbow yield no event', () => {
    for (const kind of ['grass', 'road', 'lake', 'rainbow'] as const) {
      const out = rollTileEvent({ seed: 1, stepCount: 1, cellId: 1, kind, hengeReady: false, hasScout: false })
      expect(out).toBe(null)
    }
  })

  it('henge yields fight when ready, null when on cooldown', () => {
    expect(rollTileEvent({ seed: 1, stepCount: 1, cellId: 1, kind: 'henge', hengeReady: true, hasScout: false })).toEqual({
      kind: 'fight',
      source: 'henge',
    })
    expect(rollTileEvent({ seed: 1, stepCount: 1, cellId: 1, kind: 'henge', hengeReady: false, hasScout: false })).toBe(null)
  })
  ...
  it('woods maps percentile to fight / lost / null in monotonic ranges', () => {
    ...
    expect(rollTileEvent({ seed: fightSeed, stepCount: 1, cellId: 1, kind: 'woods', hengeReady: false, hasScout: false })).toEqual({
      kind: 'fight',
      source: 'woods',
    })
    expect(rollTileEvent({ seed: lostSeed, stepCount: 1, cellId: 1, kind: 'woods', hengeReady: false, hasScout: false })).toEqual({
      kind: 'lost',
      source: 'woods',
    })
    expect(rollTileEvent({ seed: nullSeed, stepCount: 1, cellId: 1, kind: 'woods', hengeReady: false, hasScout: false })).toBe(null)
  })

  it('mountain only yields fight; never lost', () => {
    for (let seed = 1; seed <= 200; seed++) {
      const out = rollTileEvent({ seed, stepCount: 1, cellId: 1, kind: 'mountain', hengeReady: false, hasScout: false })
  ...
  it('swamp only yields lost; never fight', () => {
    for (let seed = 1; seed <= 200; seed++) {
      const out = rollTileEvent({ seed, stepCount: 1, cellId: 1, kind: 'swamp', hengeReady: false, hasScout: false })
